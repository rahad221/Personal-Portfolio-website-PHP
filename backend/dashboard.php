...
<div class="row">
    <div class="col">
        <div class="page-description">
            <h1>Dashboard</h1>
            <h2>Hello! <?=$_SESSION['auth_name']?> <span class="text-danger">Login Successfull</span></h2>
        </div>
    </div>
</div>
<div class="row">
    ...
    <div class="col-xl-6">
        <div class="card widget widget-list">
            <div class="card-header">
                <?php
                    $id = $_SESSION['auth_id'];
                    $user_count_query = "SELECT COUNT(*) AS 'Total_User' FROM `admins`";
                    $user_count_query_db = mysqli_query($db_connect, $user_count_query);
                    $total_user = mysqli_fetch_assoc($user_count_query_db)['Total_User'];
                    $user_query = "SELECT * FROM `admins` ORDER BY id DESC LIMIT 5";
                    $user_query_db = mysqli_query($db_connect, $user_query);
                ?>
                <h5 class="card-title">Total Users<span class="badge badge-success badge-style-light"><?=$total_user?> Users</span></h5>
            </div>
            <div class="card-body">
                <span class="text-muted m-b-xs d-block">showing <?=$user_query_db -> num_rows?> out of <?=$total_user?> active tasks.</span>
